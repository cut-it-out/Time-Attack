...
namespace TimeAttack
{
    public abstract class PollingPool<T> where T : Component
    {
        ...
        protected PollingPool(T prefab, Transform parentTransform, int preWarm = 0)
        {
            this.prefab = prefab;
            this.parentTransform = parentTransform;
            if (preWarm > 0)
                foreach (var item in Enumerable.Range(0, preWarm).Select((i, index) => GameObject.Instantiate(prefab, parentTransform)))
                {
